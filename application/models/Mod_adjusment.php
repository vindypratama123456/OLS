...
class Mod_adjusment extends CI_Model
{
    ...
    public function getRekapitulasiAdjusment($start_date, $end_date, $id_gudang, $order_by) 
    {
        $this->db->select('
            `b`.`id_transaksi` AS `id_transaksi`, 
            `b`.`created_date` AS `tgl_transaksi`, 
            ...
            (
                SELECT xx.hpp
                FROM master_hpp xx
                WHERE xx.id_gudang = b.asal AND xx.id_produk = d.id_product AND xx.id_periode = (SELECT `id` FROM `master_periode` where year_start = '.date('Y',strtotime($start_date)).'  ORDER BY `id` DESC LIMIT 1)
            ) AS hpp,
            (`a`.`jumlah` * (
                SELECT xx.hpp
                FROM master_hpp xx
        ...
        $this->db->join('master_gudang g', 'b.asal = g.id_gudang', 'inner');
        $this->db->where('b.id_tipe', '3');
        ...
        if ($order_by)
            ...
            $this->db->order_by('b.id_transaksi, b.created_date', 'ASC');
        return $this->db->get()->result_array();
    }

    public function get_data_product($where)
    {
        $this->db->select('a.*, b.name AS kelas');
        $this->db->from('product a');
        $this->db->join('category b', 'a.`id_category_default`=b.`id_category`', 'inner');
        $this->db->where($where);
        return $this->db->get()->result();
    }

    public function get_detail_adjusment($id_transaksi)
    {
        $this->db->select("a.*, b.*, c.name as kelas");
        $this->db->from('transaksi_detail a');
