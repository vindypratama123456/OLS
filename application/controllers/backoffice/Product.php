...
class Product extends MY_Controller
{
    ...
    public function listProduct()
    {
        ...
        $this->datatables->select('
            ...
            WHEN 1 THEN CONCAT("<span class=\"label label-success\">Ya</span>") END AS `active`
            ,CASE a.`enable` 
            WHEN 0 THEN CONCAT("<span class=\"label label-danger\">Tidak</span>") 
    ...
    public function detail_post()
    {
        ...
        try {
            if (in_array($this->adm_level, $this->backoffice_admin_area) && in_array($this->adm_level, $this->auditor_area)) {
                ...
                // Check data product category
                $check = $this->mod_product->check_category_product($id_product, $id_category_default);
                if($check <=0)
                {
                    // delete data category product in category_product table
                    $this->mod_general->deleteData("category_product", "id_product", $id_product);

                    // get id_parent category
                    $id_parent = $this->mod_product->get_id_parent_category($id_category_default)['id_parent'];

                    // add category product with id_category and id_parent
                    $data_id_category = array(
                        "id_product" => $id_product,
                        "id_category" => $id_category_default
                    );
                    $this->mod_general->addData("category_product", $data_id_category);

                    $data_id_parent = array(
                        "id_product" => $id_product,
                        "id_category" => $id_parent
                    );
                    $this->mod_general->addData("category_product", $data_id_parent);
                }
