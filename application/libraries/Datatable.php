...
class Datatable
{
    ...
    protected function get_filtering()
    {
        ...
        if ($this->request['sColumns']) {
            $mColArray = explode(',', $this->request['sColumns']);
        } else {
            $mColArray = $this->columns;
        }

        $sWhere = '';
        $sSearch = mysqli_real_escape_string($this->db->conn_id, $this->ci->input->post('sSearch'));
      ...
      //var_dump($columns);
        ...
        /*for($i = 0; $i < count($mColArray); $i++)
          if($this->request['bSearchable_' . $i] == 'true' && in_array($mColArray[$i], $columns))
            $sWhere .= $columns . " LIKE '%" . $sSearch . "%' OR ";*/
            foreach ($columns as $column) {
                if (isset($column['expression'])) {
                    $sWhere .= $column['expression'] . " LIKE '%" . $sSearch . "%' OR ";
                } else {
                    $sWhere .= $column['db'] . " LIKE '%" . $sSearch . "%' OR ";
                }
            }
        }
        ...
        for ($i = 0; $i < intval($this->request['iColumns']); $i++) {/*
          if(isset($_POST['sSearch_' . $i]) && $this->request['sSearch_' . $i] != '' && in_array($mColArray[$i], $columns))
          {
            $miSearch = explode(',', $this->request['sSearch_' . $i]);
            var_dump($miSearch);
            ...
            if (isset($_POST['sSearch_' . $i]) && $this->request['sSearch_' . $i] != '') {
                $miSearch = explode(',', $this->request['sSearch_' . $i]);
                if (count($miSearch)>1) {
                    foreach ($miSearch as $val) {
                        if (preg_match("/(<=|>=|=|<|>)(\s*)(.+)/i", trim($val), $matches)) {
                            $this->db->where($columns[$i]['db'].' '.$matches[1], $matches[3]);
                        } else {
                            $this->db->where($this->select[$mColArray[$i]].' LIKE', '%'.$val.'%');
                        }
                    }
                } elseif (isset($columns[$i]['expression'])) {
                    $this->db->where($columns[$i]['expression'].' LIKE', '%'.$miSearch[0].'%');
                } else {
                    $this->db->where($columns[$i]['db'].' LIKE', '%'.$miSearch[0].'%');
                }
            }
        }
    ...
    public function generate($as_json = true)
    {
        ...
        /*$lebih10 = ($recordsTotal < 10) and (count($data)<10);
			$recordsFiltered = ($lebih10)?count($data):$recordsTotal;*/
        //var_dump($select);
    ...
    protected function set_limit()
    {
        ...
        $limit = $this->request['iDisplayLength'];
        $start = $this->request['iDisplayStart'];

        if ($limit > 0) {
            $this->db->limit($limit, $start);
            if (isset($this->request['start']) && $this->request['length'] != -1) {
                $this->offset((int) $this->request['start'])->limit((int) $this->request['length']);
            }
        }
